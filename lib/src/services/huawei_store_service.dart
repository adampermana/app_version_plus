...
class HuaweiStoreService {
  ...
  Future<VersionInfo?> getStoreVersion(PackageInfo packageInfo) async {
    final id = huaweiId ?? packageInfo.packageName;
    final uri = Uri.https(StoreUrls.huaweiAppGallery, "/store/apps/details", {
      "id": id.toString(),
      "hl": huaweiAppGalleryCountry ?? "en_US",
    ...
    // Extract version - simplified regex pattern
    final regexp = RegExp(r'"version"\s*:\s*"([0-9]+\.[0-9]+(?:\.[0-9]+)?)"');
    final storeVersion = regexp.firstMatch(response.body)?.group(1);

    // Extract release notes
    final regexpRelease = RegExp(r'"releaseNotes"\s*:\s*"([^"]*)"');
    String? releaseNotes = regexpRelease.firstMatch(response.body)?.group(1);
    ...
    final lastUpdateDate =
        _extractLastUpdateDate(response.body, huaweiAppGalleryCountry);
    ...
    return VersionInfo.fromStore(
      ...
      releaseNotes:
          HtmlUtils.formatReleaseNotes(releaseNotes, huaweiHtmlReleaseNotes),
