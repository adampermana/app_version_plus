...
class AndroidStoreService {
  ...
  Future<VersionInfo?> getStoreVersion(PackageInfo packageInfo) async {
    final id = androidId ?? packageInfo.packageName;
    final uri = Uri.https(StoreUrls.playStore, "/store/apps/details", {
      "id": id.toString(),
    ...
    // Fallback: try alternative patterns if primary didn't work
    releaseNotes ??= _extractReleaseNotes(response.body);
  ...
  /// Extract release notes using multiple patterns
  String? _extractReleaseNotes(String body) {
    try {
      // Pattern 1: JSON-style "What's New" or "releaseNotes"
      final jsonPatterns = [
        RegExp(r'"releaseNotes"\s*:\s*"([^"]*)"', caseSensitive: false),
        RegExp(r'"whats_new"\s*:\s*"([^"]*)"', caseSensitive: false),
      ...
      // Pattern 2: Script data patterns (Google uses encoded data)
      final scriptPatterns = [
        RegExp(r'\["[^"]*","[^"]*","([^"]{50,})"'),
        RegExp(r',\[null,\[null,"([^"]{30,})"\]\]'),
        RegExp(r'"([^"]*(?:bug|fix|improve|update|new|feature)[^"]*)"',
            caseSensitive: false),
      ];
      ...
      for (final pattern in scriptPatterns) {
        ...
        for (final match in matches) {
          final notes = match.group(1);
          if (notes != null && notes.length > 50 && notes.length < 5000) {
            // Check if it looks like release notes (contains typical keywords)
            final lowerNotes = notes.toLowerCase();
            if (lowerNotes.contains('fix') ||
                lowerNotes.contains('update') ||
                lowerNotes.contains('improve') ||
                lowerNotes.contains('new') ||
                lowerNotes.contains('bug') ||
                lowerNotes.contains('version')) {
              return _cleanReleaseNotes(notes);
            }
          }
        }
      ...
      // Pattern 3: HTML "What's New" section
      final htmlPattern = RegExp(
        r"What's New.*?<[^>]+>([^<]+(?:<br\s*/?>[^<]+)*)",
        caseSensitive: false,
        dotAll: true,
      );
      final htmlMatch = htmlPattern.firstMatch(body);
      if (htmlMatch != null) {
        return _cleanReleaseNotes(htmlMatch.group(1));
      }
    } catch (e) {
      // Ignore errors
    }
  ...
  /// Clean up extracted release notes
